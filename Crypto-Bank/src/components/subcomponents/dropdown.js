...
const DropdownMenu = ({ balance, access }) => {
  const { setOperation, setType, setAmount, setBal, setCrypto } =
  ...
  let arr;
  const arr1 = [
    '0x41Cb39177A332c6dAEcEE40A5343a77411ca6712',
    '0x779877A7B0D9E8603169DdbD7836e478b4624789',
    '0xE08339cf84f40c13bB03633E30B0E77A3ba64F53',
  ];
  const arr2 = [
    '0x975C2bc54EA2e560E86845F2559d651E78F9ee41',
    '0x326C977E6efc84E512bB9C30f76E30c160eD06FB',
    '0xE6CaA60d4421F01647E894F843682047ecDeBB08',
  ];
  ...
  const handleOptionSelect = async (TokenAddress) => {
    ...
    if (!balance) {
      ...
      try {
        ...
        const jwttokendetails = jwt(jwttoken);
        if (jwttokendetails.exp > Date.now() / 1000) {
          navigate('/token');
        }
      } catch (error) {
        setLoading(false);
      ...
      if (access) {
        ...
        // amount = await BankContract.userTokenBalance(
        //   signerAddress,
        //   TokenAddress,
        // );
        ...
        const res = await fetch('http://localhost:8000/api/balance', {
          ...
          body: JSON.stringify({ signerAddress, TokenAddress, network }),
        });
  ...
  return (
    <div className="m-auto">
      ...
      {!loading && (
        <Menu
          sx={{ marginLeft: '40px', borderRadius: '20px' }}
          anchorEl={anchorEl}
          open={Boolean(anchorEl)}
          onClose={handleMenuClose}
        >
          <MenuItem
            onClick={() => handleOptionSelect(arr[0])}
            sx={{
              display: 'flex',
              justifyContent: 'center',
              alignItems: 'center',
              color: '#4072d9',
              fontWeight: 'bold',
              textShadow: '0px 4px 10px rgba(0,sf,0,0.25)',
              '&:hover': {
          ...
          <MenuItem
            onClick={() => handleOptionSelect(arr[1])}
            sx={{
              display: 'flex',
              justifyContent: 'center',
              alignItems: 'center',
          ...
          <MenuItem
            onClick={() => handleOptionSelect(arr[2])}
            sx={{
              display: 'flex',
              justifyContent: 'center',
              alignItems: 'center',
