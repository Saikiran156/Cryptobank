...
export default function Content() {
  ...
  async function MetamaskConnect(name) {
    ...
    let data = await fetch('http://localhost:8000/api/generateNonce');
    data = await data.json();
    const message = `You are requested to sign the message ${data.nonce}`;
    ...
    if (name === 'owner') {
      signerAddress === '0x967B1dC7FFC8E5D02D3aE0455863BE76B1cD412F'
        ? navigate('/owneraccess')
      ...
      const response = await fetch('http://localhost:8000/api/login', {
        ...
        body: JSON.stringify({ signerAddress, signedMessage, message }),
      });
    ...
    <div className="content flex justify-around place-content-around mb-10">
      <div className="flex w-3/5 flex-col m-16">
        <div className="flex flex-col text-slate-700 text-8xl  w-full justify-center gap-5">
          <h1>
            The <span className="text-blue">First Secure </span>Crypto
          </h1>
        ...
        <div className="btn flex text-white">
          <button
            name="owner"
            className="bg-primary rounded-full p-5 w-64 outline-none shadow-md shadow-black text-3xl font-semibold mr-20 hover:bg-least"
            onClick={() => {
              MetamaskConnect('owner');
            }}
          >
            Owner Access
          </button>
          <button
            name="user"
            className="bg-primary rounded-full p-5 w-64 outline-none shadow-md shadow-black text-3xl font-semibold hover:bg-least"
            onClick={() => {
              MetamaskConnect('user');
            }}
          >
            User Access
          </button>
      ...
      <div className="sideImage flex flex-end w-2/5 p-6 rounded-full shadow-primary mb-10">
        <div className="border-b-white rounded-full">
          <img
            class="image object-fill w-full h-full "
            src={sideImage}
            alt="img not found"
          ></img>
        </div>
